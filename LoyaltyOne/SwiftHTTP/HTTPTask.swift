...
public class HTTPTask : NSObject, NSURLSessionDelegate, NSURLSessionTaskDelegate {
   ...
   private func createRequest(url: String, method: HTTPMethod, parameters: Dictionary<String,AnyObject>!) -> (request: NSURLRequest, error: NSError?) {
    ...
    if let u = NSURL(string: urlVal.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)!) {
    ...
    */
    private func createError(code: Int) -> NSError {
        var text = "Sorry, an error occurred while retrieving information from our services"
        if code == 404 {
            text = "Sorry, the page wasn't found at this time"
        } else if code == 401 {
            text = "Sorry, the access to this service is denied"
        } else if code == -1001 {
            text = "Sorry, this is an invalid URL"
        }
